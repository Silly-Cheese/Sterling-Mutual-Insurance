...
export default function CustomerPortal({user,customerAccount,onSignOut}){
  ...
  async function requestReinstatement(c){
    await addDoc(collection(db,"serviceRequests"),{customerId:customerAccount.customerId,customerName:customer?.displayName||customerAccount.displayName,type:"reinstatement_review",details:"Customer requested reinstatement review for "+c.policyNumber,status:"submitted",policyId:c.policyId,cancellationId:c.id,submittedBy:user.uid,submittedByEmail:user.email,createdAt:serverTimestamp()});
    await load();
  }
  ...
  return <div className="customer-portal-shell">
    ...
    <main className="customer-portal-main">
      ...
      <div className="record-summary customer-account-summary"><div><span>Customer</span><strong>{customer?.displayName||customerAccount.displayName}</strong></div><div><span>Active policies</span><strong>{policies.filter(p=>p.status==="active").length}</strong></div><div><span>Open claims</span><strong>{claims.filter(c=>!["closed","denied"].includes(c.status)).length}</strong></div><div><span>Amount due</span><strong>{money(balance)}</strong></div></div>{cancellations.some(c=>c.status==="open")&&<div className="portal-warning"><ShieldAlert size={17}/><div><strong>Cancellation action pending</strong><span>Review the Cancellations tab for the effective date and amount needed to cure the account.</span></div></div>}
