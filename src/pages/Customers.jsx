...
export default function Customers({staff,onNavigate,openNew}){
  ...
  <div className="table-card"><div className="table-toolbar"><div className="search table-search"><Search size={16}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search name, email, or phone…"/></div><span>{filtered.length} records</span></div>{filtered.length===0?<div className="empty-state"><UserRound size={30}/><h3>No matching customers.</h3><p>Create a customer or adjust your search.</p></div>:<div className="customer-list">{filtered.map(c=><button className="customer-row" key={c.id} onClick={()=>setSelected(c)}><div className="avatar small">{(c.firstName?.[0]||"")+(c.lastName?.[0]||"")}</div><div><strong>{c.displayName}</strong><span>{c.email||"No email"} • {c.phone||"No phone"}</span></div><span className="status-pill prospect">{c.status}</span><span className="customer-id">{c.source==="walk_in"?"WALK-IN":c.id.slice(0,8).toUpperCase()}</span></button>)}</div>}</div>
