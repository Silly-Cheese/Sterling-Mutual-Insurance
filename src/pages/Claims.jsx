...
function ClaimDetail({claim,staff,staffList,payments,contacts,estimates,events,parties,coverages,onClose,patchClaim,assignClaim,referSIU,reload}){
  ...
  async function requestReserveApproval(){
    await addDoc(collection(db,"approvals"),{actionType:"large_reserve",title:"Large claim reserve",summary:claim.claimNumber+" • "+money(totalReserve)+" proposed reserve",recordId:claim.id,customerId:claim.customerId,requestedAmount:totalReserve,status:"pending",requestedBy:staff.id,requestedByName:staff.displayName,createdAt:serverTimestamp()});
    await addEvent(claim.id,"reserve.approval.requested","Large reserve sent for supervisor approval",{amount:totalReserve});
  }
