...
export default function Claims({staff,initialCustomerId,openNew}){
  ...
  return <section className="content">
    ...
    {selected&&<ClaimDetail claim={selected} staff={staff} staffList={staffList} payments={payments.filter(p=>p.claimId===selected.id)} contacts={contacts.filter(x=>x.claimId===selected.id)} estimates={estimates.filter(x=>x.claimId===selected.id)} events={events.filter(x=>x.claimId===selected.id)} parties={parties.filter(x=>x.claimId===selected.id)} coverages={coverages.filter(x=>x.claimId===selected.id)} addEvent={addEvent} onClose={()=>setSelected(null)} patchClaim={patchClaim} assignClaim={assignClaim} referSIU={referSIU} reload={load}/>}
  </section>
}

function ClaimDetail({claim,staff,staffList,payments,contacts,estimates,events,parties,coverages,addEvent,onClose,patchClaim,assignClaim,referSIU,reload}){
