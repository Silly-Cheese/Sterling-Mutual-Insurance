...
export default function Claims({staff,initialCustomerId,openNew}){
  ...
  async function load(){
    ...
    setStaffList(s.filter(x=>x.status==="active"));setPayments(pay);setContacts(con);setEstimates(est);setEvents(ev.sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0)));setParties(par);setCoverages(cov);
  ...
  async function createClaim(e){
    ...
    try{
      ...
      const ref=await addDoc(collection(db,"claims"),{
        ...
        lossDate:form.lossDate,lossType:form.lossType,description:form.description,claimedAmount:Number(form.claimedAmount||0),reportedBy:form.reportedBy,reportMethod:form.reportMethod,priority:form.priority,
...
function ClaimDetail({claim,staff,staffList,payments,contacts,estimates,events,parties,coverages,onClose,patchClaim,assignClaim,referSIU,reload}){
  ...
  return <div className="modal-backdrop"><div className="modal claim-detail"><div className="modal-head"><div><div className="eyebrow">CLAIM FILE</div><h2>{claim.claimNumber}</h2><p>{claim.customerName} • {claim.policyNumber}</p></div><button onClick={onClose}><X/></button></div>
    <div className="claim-summary"><div><span>Stage</span><strong>{(claim.stage||"intake").replaceAll("_"," ")}</strong></div><div><span>Coverage</span><strong>{claim.coverageStatus}</strong></div><div><span>Reserve</span><strong>{money(claim.reserveAmount)}</strong></div><div><span>Paid</span><strong>{money(paid)}</strong></div></div>
    <div className="record-tabs">{[["summary","Summary"],["parties","Parties"],["contacts","Contacts"],["coverage","Coverage"],["investigation","Investigation"],["financials","Financials"],["timeline","Timeline"],["closure","Closure"]].map(([k,l])=><button key={k} className={tab===k?"active":""} onClick={()=>setTab(k)}>{l}</button>)}</div>
    ...
    {tab==="closure"&&<div className="claim-close-panel"><div className="eyebrow">CLOSURE CHECK</div><h3>Is this file ready to close?</h3><div className="close-checks">{closeChecks.map(([label,ok])=><div className={ok?"ok":"missing"} key={label}><ClipboardCheck size={16}/><span>{label}</span><strong>{ok?"Ready":"Needs action"}</strong></div>)}</div>{can(staff,PERMISSIONS.CLAIM_CLOSE)&&claim.status!=="closed"&&<button className="primary" onClick={closeClaim}>Close claim file</button>}{claim.status==="closed"&&can(staff,PERMISSIONS.CLAIM_UPDATE)&&<div className="reopen-box"><label>Reason to reopen<input value={reopenReason} onChange={e=>setReopenReason(e.target.value)} placeholder="Required reason"/></label><button className="secondary" onClick={reopenClaim}>Reopen claim</button></div>}</div>}
  </div></div>
}
