...
export default function Claims({staff,initialCustomerId,openNew}){
  const [claims,setClaims]=useState([]),[policies,setPolicies]=useState([]),[staffList,setStaffList]=useState([]),[payments,setPayments]=useState([]),[contacts,setContacts]=useState([]),[estimates,setEstimates]=useState([]),[events,setEvents]=useState([]),[open,setOpen]=useState(false),[form,setForm]=useState(empty),[selected,setSelected]=useState(null),[saving,setSaving]=useState(false);

  async function safe(n){try{return (await getDocs(collection(db,n))).docs.map(d=>({id:d.id,...d.data()}))}catch{return []}}
  async function load(){
    const [c,p,s,pay,con,est,ev]=await Promise.all(["claims","policies","staff","claimPayments","claimContacts","claimEstimates","claimEvents"].map(safe));
    ...
    setStaffList(s.filter(x=>x.status==="active"));setPayments(pay);setContacts(con);setEstimates(est);setEvents(ev.sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0)));
  ...
  return <section className="content">
    ...
    {selected&&<ClaimDetail claim={selected} staff={staff} staffList={staffList} payments={payments.filter(p=>p.claimId===selected.id)} contacts={contacts.filter(x=>x.claimId===selected.id)} estimates={estimates.filter(x=>x.claimId===selected.id)} events={events.filter(x=>x.claimId===selected.id)} onClose={()=>setSelected(null)} patchClaim={patchClaim} assignClaim={assignClaim} referSIU={referSIU} reload={load}/>}
  </section>
}

function ClaimDetail({claim,staff,staffList,payments,contacts,estimates,events,onClose,patchClaim,assignClaim,referSIU,reload}){
  ...
  return <div className="modal-backdrop"><div className="modal claim-detail"><div className="modal-head"><div><div className="eyebrow">CLAIM FILE</div><h2>{claim.claimNumber}</h2><p>{claim.customerName} • {claim.policyNumber}</p></div><button onClick={onClose}><X/></button></div>
    ...
    <div className="record-tabs">{[["summary","Summary"],["contacts","Contacts"],["investigation","Investigation"],["financials","Financials"],["timeline","Timeline"],["closure","Closure"]].map(([k,l])=><button key={k} className={tab===k?"active":""} onClick={()=>setTab(k)}>{l}</button>)}</div>
