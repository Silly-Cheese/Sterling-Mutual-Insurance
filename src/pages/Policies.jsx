...
export default function Policies({staff,onNavigate,initialCustomerId}){
  ...
  async function createTransaction(p,type,summary,details={}){
    await addDoc(collection(db,"policyTransactions"),{policyId:p.id,policyNumber:p.policyNumber,customerId:p.customerId,customerName:p.customerName,type,summary,details,status:"completed",createdBy:staff.id,createdByName:staff.displayName,createdAt:serverTimestamp()});
  }

  async function markNonrenewal(p){
    const reason=window.prompt("Reason for nonrenewal?");
    if(!reason)return;
    await updateDoc(doc(db,"policies",p.id),{status:"nonrenewal_pending",nonrenewalReason:reason,nonrenewalAtExpiration:true,updatedAt:serverTimestamp(),updatedBy:staff.id});
    await addDoc(collection(db,"documents"),{customerId:p.customerId,policyId:p.id,policyNumber:p.policyNumber,type:"nonrenewal_notice",title:"Notice of Nonrenewal",status:"available",effectiveDate:p.expirationDate,reason,createdAt:serverTimestamp(),createdBy:staff.id});
    await createTransaction(p,"nonrenewal","Policy marked for nonrenewal",{reason,expirationDate:p.expirationDate});
    setSelected({...p,status:"nonrenewal_pending",nonrenewalReason:reason});await load();
  }

  async function rewritePolicy(p){
    const ref=await addDoc(collection(db,"quotes"),{customerId:p.customerId,customerName:p.customerName,product:p.product,coverageTier:"standard",assetDescription:assetMap[p.id]?.description||"",basePremium:Number(p.monthlyPremium||0),deductible:Number(p.deductible||0),liabilityLimit:Number(p.liabilityLimit||0),propertyDamageLimit:Number(p.propertyDamageLimit||0),monthlyPremium:Number(p.monthlyPremium||0),sixMonthPremium:Number(p.termPremium||0),status:"draft",rewriteOfPolicyId:p.id,rewriteOfPolicyNumber:p.policyNumber,underwritingLevel:"unreviewed",createdBy:staff.id,createdAt:serverTimestamp(),updatedAt:serverTimestamp()});
    await updateDoc(doc(db,"policies",p.id),{rewritePending:true,rewriteQuoteId:ref.id,updatedAt:serverTimestamp()});
    await createTransaction(p,"rewrite_started","Policy rewrite started",{quoteId:ref.id});
    setSelected({...p,rewritePending:true,rewriteQuoteId:ref.id});await load();
  }

  async function correctPolicy(p){
    setEditForm({monthlyPremium:String(p.monthlyPremium??""),termPremium:String(p.termPremium??""),deductible:String(p.deductible??""),liabilityLimit:String(p.liabilityLimit??""),propertyDamageLimit:String(p.propertyDamageLimit??""),effectiveDate:p.effectiveDate||"",expirationDate:p.expirationDate||"",reason:"Administrative correction"});
    setEditing(true);
  }
  ...
  return <section className="content">
    ...
    {selected&&<div className="modal-backdrop"><div className="modal claim-detail"><div className="modal-head"><div><div className="eyebrow">POLICY RECORD</div><h2>{selected.policyNumber}</h2></div><button onClick={()=>setSelected(null)}><X/></button></div>
      <div className="record-tabs"><button className={mode==="policy"?"active":""} onClick={()=>setMode("policy")}>Policy</button><button className={mode==="versions"?"active":""} onClick={()=>setMode("versions")}>Version history</button><button className={mode==="endorsements"?"active":""} onClick={()=>setMode("endorsements")}>Endorsements</button><button className={mode==="transactions"?"active":""} onClick={()=>setMode("transactions")}>Transactions</button><button className={mode==="cancellation"?"active":""} onClick={()=>setMode("cancellation")}>Cancellation</button>{can(staff,PERMISSIONS.POLICY_RENEW)&&<button className={mode==="renewal"?"active":""} onClick={()=>prepareRenewal(selected)}>Renewal</button>}</div>
      ...
      {mode==="transactions"&&<div className="timeline">{transactions.filter(t=>t.policyId===selected.id).length===0?<div className="empty-state compact-empty">No policy transactions recorded.</div>:transactions.filter(t=>t.policyId===selected.id).sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0)).map(t=><div key={t.id}><span className="timeline-dot"></span><div><strong>{t.type.replaceAll("_"," ")} — {t.summary}</strong><p>{t.status}</p><small>{t.createdByName||"Staff"} • {t.createdAt?.toDate?t.createdAt.toDate().toLocaleString():"Recorded"}</small></div></div>)}</div>}
