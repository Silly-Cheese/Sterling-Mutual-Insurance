...
export default function Billing({staff,initialPolicyId}){
  ...
  function filteredCustomerInvoices(customerId){
    return customerInvoices(customerId).filter(i=>{
      const q=invoiceSearch.trim().toLowerCase();
      const text=[i.invoiceNumber,i.description,i.policyNumber,i.status,i.dueDate].filter(Boolean).join(" ").toLowerCase();
      const matchesSearch=!q||text.includes(q);
      const matchesFilter=invoiceFilter==="all"||invoiceFilter==="open"&&!["paid","void"].includes(i.status)||invoiceFilter==="paid"&&i.status==="paid"||invoiceFilter==="overdue"&&!["paid","void"].includes(i.status)&&i.dueDate<today();
      return matchesSearch&&matchesFilter;
    }).sort((a,b)=>String(b.dueDate).localeCompare(String(a.dueDate)));
  }
  function filteredCustomerTransactions(customerId){
    return tx.filter(t=>t.customerId===customerId).filter(t=>{
      const q=paymentSearch.trim().toLowerCase();
      const text=[t.type,t.policyNumber,t.note,t.createdByName,t.status].filter(Boolean).join(" ").toLowerCase();
      return (!q||text.includes(q))&&(paymentFilter==="all"||t.type===paymentFilter);
    });
  }
  function nextInvoice(customerId){
    return customerInvoices(customerId).filter(i=>!["paid","void"].includes(i.status)).sort((a,b)=>String(a.dueDate).localeCompare(String(b.dueDate)))[0]||null;
  }
  ...
  return <section className="content">
    ...
    {selectedCustomer&&!selected&&<div className="modal-backdrop"><div className="modal claim-detail customer-billing-detail"><div className="billing-control-deck"><div className="modal-head"><div><div className="eyebrow">CUSTOMER BILLING ACCOUNT</div><h2>{selectedCustomer.customerName}</h2><p>One financial relationship across all Sterling Mutual policies.</p></div><button onClick={()=>setSelectedCustomer(null)}><X/></button></div><div className="billing-account-hero"><div><span>Total open</span><strong>{money(selectedCustomer.openBalance)}</strong></div><div><span>Past due</span><strong>{money(selectedCustomer.pastDue)}</strong></div><div><span>Policies</span><strong>{selectedCustomer.policies.length}</strong></div><div><span>Account health</span><strong>{selectedCustomer.pastDue>0?"Needs attention":"Current"}</strong></div></div>
      ...
      {customerTab==="overview"&&<div className="customer-billing-overview">
        ...
        <div className="billing-quick-actions">
          <button onClick={()=>setCustomerTab("invoices")}><FileText size={17}/><span><strong>Create / view invoices</strong><small>Bill the customer or review invoice status</small></span></button>
          <button onClick={()=>setCustomerTab("payments")}><CreditCard size={17}/><span><strong>Payment history</strong><small>Review payments, credits, refunds, and adjustments</small></span></button>
          <button onClick={()=>setCustomerTab("policies")}><ReceiptText size={17}/><span><strong>Policy billing</strong><small>Open detailed billing for a specific policy</small></span></button>
          {selectedCustomer.pastDue>0&&<button className="attention" onClick={()=>setCustomerTab("invoices")}><TriangleAlert size={17}/><span><strong>Past-due balance</strong><small>{money(selectedCustomer.pastDue)} requires attention</small></span></button>}
        </div>
      </div>}
